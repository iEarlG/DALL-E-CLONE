...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {  
    ...
    if(form.prompt && form.photo) {
      ...
      try { 
        const response = await fetch("https://dall-e-2-clone.onrender.com/api/v1/post", {
          method: 'POST',
          headers: {
  ...
  const generateImage = async () => {
    if(form.prompt) { 
      try {
        ...
        const response = await fetch('https://dall-e-2-clone.onrender.com/api/v1/dalle', { 
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      <form action="" className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        ...
        <div className="mt-10">
          ...
          <button 
            ...
            >
              {loading ? 'Loading Image...' : 'Share Image'}
            </button>
        </div>
      </form>
    </section>
